...
import {
  ...
  Calendar,
  Filter,
  FileSpreadsheet, // เพิ่มไอคอน Excel
} from "lucide-react";

// --- เพิ่ม Library สำหรับ Excel (อย่าลืมลง Dependencies: xlsx) ---
...
// ==========================================
// 📍 ตั้งค่าพิกัดวิทยาลัย (วิทยาลัยเทคนิคนนทบุรี)
// ==========================================
...
export default function PhotoAttendanceSystem() {
  ...
  const exportToExcel = (student: any) => {
    ...
    const data = studentRecords.map((record) => ({
      วันที่: formatDate(record.checkInTime),
      เวลา: formatTime(record.checkInTime),
      สถานะ: record.status === "late" ? "มาสาย" : "มาทันเวลา",
      สถานที่: record.isOffCampus ? "นอกพื้นที่" : "ในวิทยาลัย",
      ระยะห่าง: Math.round(record.distance || 0) + " เมตร",
    }));

    // สร้าง Workbook และ Worksheet ด้วย xlsx library
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Attendance");

    // ดาวน์โหลดไฟล์
    XLSX.writeFile(wb, `ประวัติการเช็คชื่อ_${student.fullName}.xlsx`);
  };
              ...
              <div className="bg-white rounded-xl">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => exportToExcel(viewingHistoryStudent)}
